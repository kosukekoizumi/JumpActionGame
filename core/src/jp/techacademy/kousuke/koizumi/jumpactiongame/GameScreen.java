package jp.techacademy.kousuke.koizumi.jumpactiongame;
...
public class GameScreen extends ScreenAdapter {
    ...
    static final float WORLD_HEIGHT = 15 * 3; // ★3画面分登れば終了（テスト環境にて）
    ...
    Sound soundExplosion = Gdx.audio.newSound(Gdx.files.internal("sounddata/explosion.mp3")); //★Enemy衝突時効果音
    Sound soundJump = Gdx.audio.newSound(Gdx.files.internal("sounddata/jump.mp3")); //★得点時効果音
    Sound soundPoint = Gdx.audio.newSound(Gdx.files.internal("sounddata/point.mp3")); //★得点時効果音
    Sound soundFalling = Gdx.audio.newSound(Gdx.files.internal("sounddata/falling.mp3")); //★Player落下時効果音
    Sound soundGoal = Gdx.audio.newSound(Gdx.files.internal("sounddata/goal.mp3")); //★Player落下時効果音
    ...
    public GameScreen(JumpActionGame game) {
        ...
        mPrefs = Gdx.app.getPreferences("jp.techacademy.kousuke.koizumi.jumpactiongame");
    ...
    private void createStage() {
        ...
        while (y < WORLD_HEIGHT - 5) {
            ...
            if (mRandom.nextFloat() > 0.5f) { // ★出現率変更 0.6→0.5
                ...
                mStars.add(star);
            } else if (mRandom.nextFloat() < 0.3f) { //★出現率設定
    ...
    private void updateGameOver() {
        if (Gdx.input.justTouched()) {
            ...
            soundExplosion.dispose(); //★効果音破棄
            soundJump.dispose(); //★効果音破棄
            soundPoint.dispose(); //★効果音破棄
            soundFalling.dispose(); //★効果音破棄
            soundGoal.dispose(); //★効果音破棄
